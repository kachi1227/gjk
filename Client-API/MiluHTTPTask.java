...
import com.milu.Application;
import com.milu.Constants;
import com.milu.R;
import com.milu.net.MiluHttpRequest.DBHttpUploadFile;
...
public abstract class MiluHTTPTask extends HTTPTask {
	...
	protected void executeWithJson(String uri, JSONObject json) {
		...
		try {
			...
			String token = Application.get().getPreferences().getString(Constants.PREF_TOKEN, null);
			
			
			if (token != null && shouldSignRequest()) {
				req.addHeader("sos-sig", createSignature(json.toString()));
				req.addHeader("Milu-token", token);
	...
	public TaskResult handleSuccessfulResponse(MiluHttpRequest.DBHttpResponse response) throws Exception {
		try {
			...
			if (!json.getBoolean("success")) {
				if (json.has("errorCode") && json.getInt("errorCode") == 1 && shouldLogOutOnInvalidSession()) {
					new Handler(Application.get().getMainLooper()).post(new Runnable() {
						@Override
						public void run() {
							Application.get().logout();
							Toast.makeText(Application.get(), R.string.error_invalid_session, Toast.LENGTH_LONG).show();	
						}
					});
					return null;
				}
